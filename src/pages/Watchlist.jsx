...
const Watchlist = () => {
  ...
  const handleClearAll = async () => {
    setClearing(true);
    const res = await fetch(`${API_URL}/watchlist/clear`, {
      method: "DELETE",
      credentials: "include",
    });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818] px-4 sm:px-6 md:px-8 py-6 md:py-10">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="bg-[#232323] border border-[#444] rounded-2xl p-6 sm:p-8 max-w-sm w-full flex flex-col gap-4 shadow-2xl">
            ...
            <div className="flex gap-3 mt-2">
              <button onClick={() => setShowConfirm(false)} className="flex-1 py-2.5 rounded-xl border-2 border-[#444] text-white font-semibold hover:bg-[#333] transition text-sm">
                Cancel
              </button>
              <button onClick={handleClearAll} disabled={clearing} className="flex-1 py-2.5 rounded-xl bg-[#e50914] text-white font-semibold hover:bg-red-700 transition text-sm disabled:opacity-60">
      ...
      {watchlist.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
          {watchlist.map((item) => (
            <WatchlistCard key={`${item.mediaType}-${item.mediaId}`} item={item} onRemove={handleRemove} />
          ))}
        </div>
      )}
    </div>
