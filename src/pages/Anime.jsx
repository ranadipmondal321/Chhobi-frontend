...
const Anime = () => {
  ...
  useEffect(() => {
    tmdb.getAnime().then(data => { setAnime(data.results); setLoading(false); });
  }, []);
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818]">
      ...
      <p className="text-[#888] text-sm">Loading Anime...</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818] px-8 py-10">
      <h1 className="text-white text-3xl font-extrabold mb-8 tracking-tight drop-shadow-lg">Anime</h1>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-5">
        {anime.map(item => <AnimeCard key={item.id} item={item} />)}
      </div>
    </div>
  );
};
...
const AnimeCard = ({ item }) => {
  ...
  return (
    <Link to={`/tv/${item.id}`} onClick={() => addToHistory({ mediaId: String(item.id), mediaType: "tv", title: item.name, poster_path: item.poster_path, release_date: item.first_air_date })}>
      <div className={`rounded-2xl overflow-hidden cursor-pointer bg-[#181818] border border-[#333] shadow-lg transition-all duration-200 ${hovered ? "scale-105 shadow-2xl border-[#e50914]/60" : "scale-100"}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
        <div className="relative w-full aspect-[2/3] overflow-hidden">
          <img src={tmdb.image(item.poster_path)} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
        ...
        <div className="px-3 py-2">
          ...
          {item.first_air_date && <p className="text-[#999] text-xs mt-0.5">{new Date(item.first_air_date).getFullYear()}</p>}
        </div>
      </div>
    </Link>
  );
