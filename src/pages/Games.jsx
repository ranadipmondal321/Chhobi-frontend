...
const Games = () => {
  ...
  useEffect(() => {
    tmdb.getGames().then(data => { setGames(data.results); setLoading(false); });
  }, []);
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818]">
      ...
      <p className="text-[#888] text-sm">Loading Games...</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818] px-8 py-10">
      <h1 className="text-white text-3xl font-extrabold mb-8 tracking-tight drop-shadow-lg">Games</h1>
...
const GameCard = ({ game }) => {
  ...
  return (
    <Link to={`/movie/${game.id}`} onClick={() => addToHistory({ mediaId: String(game.id), mediaType: "movie", title: game.title, poster_path: game.poster_path, release_date: game.release_date })}>
      <div className={`rounded-2xl overflow-hidden cursor-pointer bg-[#181818] border border-[#333] shadow-lg transition-all duration-200 ${hovered ? "scale-105 shadow-2xl border-[#e50914]/60" : "scale-100"}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
        <div className="relative w-full aspect-[2/3] overflow-hidden">
          <img src={tmdb.image(game.poster_path)} alt={game.title} className="w-full h-full object-cover" loading="lazy" />
          <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#181818] to-transparent pointer-events-none" />
        </div>
        <div className="px-3 py-2">
          <p className="text-white text-sm font-semibold leading-snug truncate">{game.title}</p>
          {game.release_date && <p className="text-[#999] text-xs mt-0.5">{new Date(game.release_date).getFullYear()}</p>}
        </div>
      </div>
    </Link>
  );
